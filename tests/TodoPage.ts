...
export class TodoPage{
    ...
    async goto(){
        await this.page.goto('https://demo.playwrigth.dev/todomvc')
    }
    ...
    async removeAll(text: string): Promise<void>{
        while(await this.page.getByTestId(this.todoItem).count() > 0){
            this.page.getByTestId(this.todoItem).first().hover()
            this.page.getByTestId(this.todoItem).first().getByLabel('Delete').click()
        }
    }
}